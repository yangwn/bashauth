...
import com.base.privilege.biz.IDepartmentService;
import com.base.privilege.mapper.DepartmentMapper;
...
import com.base.privilege.utils.StringUtil;
...
public class DepartmentServiceImpl implements IDepartmentService {
	...
	public String getDepartmentTree() {
		...
		for (Department d : departmentList) {
			...
			i++;
			sb.append("{").append("\"id\":\"").append(d.getId()).append("\"").append(",\"parent\":\"")
					.append(d.getParentId() == 0 ? "#" : d.getParentId()).append("\"").append(",\"text\":\"")
					.append(d.getName()).append("\"").append(",\"li_attr\":{\"sortNo\":").append(d.getSortNo())
					.append("}");;
			// 前两个级别默认打开
			if (level <= 3) {
	...
	/**
	 * 这种写法和直接对所有部门列表进行循环的写法比较 优点是：更少的循环次数，所以也就是更少的CPU计算和更快的返回时间；缺点是：更多的内存占用
	 * 也可以看做是一种空间换时间
	 * 
	...
	private List<Department> buildDepartmentListForOption(List<Department> descendantDepartments,
		...
		if (descendantDepartments == null || descendantDepartments.size() == 0 || immediateDepartments == null
				|| immediateDepartments.size() == 0) {
			return null;
		}
		...
		for (Department department : immediateDepartments) {
			...
			if (tempDescendantDepartment != null && tempDescendantDepartment.size() > 0
				...
				List<Department> sub = buildDepartmentListForOption(tempDescendantDepartment, tempImmediateDepartment,
						department.getStructure());
